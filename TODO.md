...
- [x] **Nenhum teste automatizado.** ~~Não havia projeto de testes na solução.~~ Criado `Service.Tests` (xUnit + NSubstitute + EF Core InMemory), cobrindo `UserService`/`AuthenticateService` (fakes) e `BaseRepository`/`UserRepository` (InMemory) — 38 testes, todos passando. Ainda falta cobertura de `UserController`/`ExceptionMiddleware`/`ApiResponseWrapperFilter` (camada HTTP) e CI rodando isso automaticamente. (`Service.Tests/`)
...
42 warnings no total, agrupados por código e marcados com gravidade.
...
- [ ] **[WARNING] CS8618 — Gravidade: Baixa.** Propriedades `string` não anuláveis sem valor garantido ao sair do construtor — faltam os modificadores `required` ou tornar as propriedades anuláveis. Afeta toda entidade/DTO de todo módulo (`User`, `Role`, `Center`, e qualquer um futuro) — cresce linearmente a cada módulo novo, então não vale mais listar arquivo por arquivo aqui. Vale decidir um padrão único (ex.: sempre `required string` nas entidades/DTOs) e aplicar de uma vez, em vez de ir corrigindo módulo a módulo.
