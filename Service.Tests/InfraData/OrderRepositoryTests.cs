...
public class OrderRepositoryTests
{
    private static OrderRepository CreateSut()
    {
        ...
        context.Order.AddRange(
            ...
            new Order { Id = 6, OrderNumber = "DEST1-PRODUCT2", IdProduct = 2, IdDestinyCenter = 1, Quantity = 10, DeliveredQuantity = 0, MeasurementUnit = "UN", IsInbound = true, IsOutbound = false, IsFictional = false });
        context.SaveChanges();
    ...
    public async Task GetFilteredAsync_ReturnsEverythingNonDeleted_WhenNoFilterIsGiven()
    {
        ...
        var result = await repository.GetFilteredAsync(null, null, null, null, null, null, 1, 10);
    ...
    public async Task GetFilteredAsync_FiltersByIdDestinyCenter()
    {
        ...
        var result = await repository.GetFilteredAsync(1, null, null, null, null, null, 1, 10);
    ...
    public async Task GetFilteredAsync_FiltersByIdOriginCenter()
    {
        ...
        var result = await repository.GetFilteredAsync(null, 1, null, null, null, null, 1, 10);

        Assert.Equal("ORIGIN1-OUT-REAL", Assert.Single(result).OrderNumber);
    }

    [Fact]
    public async Task GetFilteredAsync_FiltersByIdProduct()
    {
        var repository = CreateSut();

        var result = await repository.GetFilteredAsync(null, null, 2, null, null, null, 1, 10);

        Assert.Equal("DEST1-PRODUCT2", Assert.Single(result).OrderNumber);
    }
    ...
    public async Task GetFilteredAsync_FiltersByFictionalIsInboundIsOutbound()
    {
        ...
        var fictionalOnly = await repository.GetFilteredAsync(null, null, null, true, null, null, 1, 10);
        Assert.Equal("DEST1-IN-FICTIONAL", Assert.Single(fictionalOnly).OrderNumber);

        var outboundOnly = await repository.GetFilteredAsync(null, null, null, null, null, true, 1, 10);
        Assert.Equal("ORIGIN1-OUT-REAL", Assert.Single(outboundOnly).OrderNumber);

        var inboundNonFictional = await repository.GetFilteredAsync(null, null, null, false, true, null, 1, 10);
        Assert.Equal(3, inboundNonFictional.TotalCount);
