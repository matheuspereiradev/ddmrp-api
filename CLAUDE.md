...
- **Service.Domain** — entities (`Entities/BaseEntity.cs`, `Entities/User.cs`, `Entities/Role.cs`, `Entities/Center.cs`), repository interfaces (`Interfaces/IBaseRepository.cs`, `IUserRepository.cs`, `IRoleRepository.cs`, `ICenterRepository.cs`), the `IAuthenticate`/`ICurrentUserService` account interfaces, and `Pagination/PagedList.cs`. No dependencies on other projects.
- **Service.Application** — DTOs (`DTOs/User/*`, `DTOs/Role/*`, `DTOs/Center/*`), service interfaces (`Interfaces/IBaseService.cs`, `IUserService.cs`, `IRoleService.cs`, `ICenterService.cs`, `IAuthenticateService.cs`), and service implementations (`Services/BaseService.cs`, `UserService.cs`, `RoleService.cs`, `CenterService.cs`, `AuthenticateService.cs`). Depends only on Domain. Custom exceptions (`Exceptions/AppException.cs` and its subclasses) are thrown here and translated to HTTP status codes by the API's exception middleware.
- **Service.Infra.Data** — `Context/ApplicationDbContext.cs` (Npgsql), EF entity configurations (`EntitiesConfiguration/`), repository implementations (`Repositories/BaseRepository.cs`, `UserRepository.cs`, `RoleRepository.cs`, `CenterRepository.cs`), `Identity/AuthenticateProvider.cs` (JWT generation), `Helpers/PaginationHelper.cs`, and `Migrations/`.
...
- **Service.API** — ASP.NET Core host: `Program.cs` wires Swagger (with bearer auth), CORS, and `AddInfrastructure()`; `Controllers/UserController.cs`, `Controllers/RoleController.cs`, `Controllers/CenterController.cs`; `Middleware/ExceptionMiddleware.cs` (global try/catch mapping exceptions to status codes); `Filters/ApiResponseWrapperFilter.cs` (wraps every response in `ApiResponseDto<T>`); `Models/ApiResponseDto.cs`, `PaginationParams.cs`, `PaginationHeader.cs`, `UserLogin.cs`; `Extensions/HttpExtensions.cs` (adds a `Pagination` response header).
- **Service.Tests** — xUnit + NSubstitute + EF Core InMemory. `Application/` unit-tests services (`UserService`, `RoleService`, `CenterService`, `AuthenticateService`) against faked repositories/`IAuthenticate`; `InfraData/` tests `BaseRepository`/`UserRepository` against a real `ApplicationDbContext` on the InMemory provider, with a faked `ICurrentUserService`. New modules get at minimum a `<Entity>ServiceTests.cs` mirroring `RoleServiceTests.cs`/`CenterServiceTests.cs` — no separate repository test is needed unless the module adds custom repository methods (`BaseRepository<T>` is already covered generically). No HTTP-layer tests yet (controllers, `ExceptionMiddleware`, `ApiResponseWrapperFilter`) and no CI wired to run this automatically.
